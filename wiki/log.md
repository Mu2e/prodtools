...
- 2026-09-10 NERSC IRI feasibility assessed: HEPCloud route needs only a `site` knob in jobsub_argv; IRI API route blocked on interactive Globus tokens and missing worker credmon token; Perlmutter cvmfs now mounts mu2e.opensciencegrid.org live. See [[nersc-iri-feasibility]].
- 2026-09-10 NERSC IRI probe ran: sfapi red client authenticates unattended (Globus rejected), m4599 visible, CFS ops + Slurm submit + status work, cvmfs mounted natively, EL9 container pulls on node; adapter bug: >1 volume_mounts breaks podman-hpc; inline bash form escapes the container. See [[nersc-iri-feasibility]].
- 2026-09-10 IRI job 58177878: Mu2e stack (muse setup SimJob MDC2025aw, g4test_03 2 events) ran to art exit 0 on Perlmutter inside fnal-wn-el9 via apptainer-from-cvmfs, launched through api.iri.nersc.gov. See [[nersc-iri-feasibility]].
- 2026-09-11 IRI job 58197742: g4bl (prodtools worker recipe, beamkit deck e470313, 100 events) ran to exit 0 on Perlmutter inside fnal-wn-el9 via apptainer-from-cvmfs; warning summary identical to the Fermilab run. Both prodtools runners proven at NERSC. See [[nersc-iri-feasibility]].
- 2026-09-12 beamkit NERSC backend built (15-task plan, 320 tests) and live-verified on Perlmutter: Slurm 58227570 two g4bl indices, 58227941 beam file; API quirks recorded (whoami numeric id, mkdir not -p, Idempotency-Key 501). See [[nersc-iri-feasibility]] and [[beamkit]].
- 2026-09-12 beamkit NERSC backend final review: 4 Important fixes (IriError wrapping, owner/base_dir validation, record-driven slicing, shlex on node), branch v1 at cc4b964, 332 tests. See [[nersc-iri-feasibility]].
- 2026-09-12 beamkit cvmfs installer + launcher (dc45f75), mcp pinned <2 (01985f6); rehearsal install passed. Decision: cvmfs per-user, not central HTTP. See [[nersc-iri-feasibility]].
- 2026-09-12 beamkit v0.3.0 published to /cvmfs/mu2e.opensciencegrid.org/bin/beamkit (first cvmfs release, repo made public); propagated in ~25 min; MCP over stdio from the cvmfs launcher reports fermilab and nersc backends available. See [[nersc-iri-feasibility]].
- 2026-09-13 IRI v2 adapter down since 2026-09-12 16:36 CDT (all facility endpoints 500, identity fine, new red and amber clients same; ticket to NERSC). beamkit gained `transport = "sfapi"` (legacy Superfacility API v1.2, same client): live run G4blSfapi.e470313-001 complete via v1.2; shared-qos rule confirmed (qos=shared on Slurm). See [[nersc-iri-feasibility]].
- 2026-09-13 beamkit `fetch_outputs(run_id, dest, kind)` (22e6213): NERSC nts/beam files to a local dir via API download, 5 MB/file cap on both APIs, sfapi binary path verified live, IRI v2 binary unverified (refuses). See [[nersc-iri-feasibility]].
- 2026-09-14 IRI v2 adapter back (iri-ping all OK 21:36 CDT; down since 2026-09-12 16:36 CDT). Verified v2 filesystem/download cannot carry a ROOT file (task fails, pydantic string_unicode): fetch_outputs stays sfapi-only. See [[nersc-iri-feasibility]].
- 2026-09-14 IRI v2 end-to-end re-verified after the outage: G4blIri.e470313 (Slurm 58332220, transport iri, shared qos) 2/2 outputs in 4m05s. fetch_outputs still needs sfapi (v2 download cannot carry binary). See [[nersc-iri-feasibility]].
- 2026-09-15 beamkit 0.4.0: prodtools driven over MCP (stdio children), cvmfs release retired, uvx install; prodtools gains locate_file/dataset_files (branch mcp-locate-and-dataset-files). Live Fermilab submit blocked: ops-021 (py3.12) + setup OfflineOps (six 1.11) breaks json2jobdef's samweb import since 2026-09-12; affects prodtools-write too. See [[nersc-iri-feasibility]].
- 2026-09-15 push_cnf six.moves fix: runner does `setup OfflineOps` before `muse setup ops` (8785ad2); beamkit 0.4.0 live Fermilab submit verified through the real prodtools-write child (G4blMcp.e470313, campaign 9, cluster 93730445). Proper fix: drop ifdhc/sam_web_client setupRequired from OfflineOps' table. See [[nersc-iri-feasibility]].
- 2026-09-15 worker script had the same six.moves trap: cluster 93730445 died at import on both workers. runjob.sh fixed (69b9b90, OfflineOps before muse). Verified with the dev tree shipped: G4blMcpFix.e470313 campaign 10 cluster 29960571, 2/2 outputs, beam file built through beamkit 0.4.0 over MCP. Production untouched (last mu2epro submission 2026-09-03) but any tick before a prodtools cvmfs release with both fixes would fail every job. See [[nersc-iri-feasibility]].
- 2026-09-17 read-only prodtools MCP gained `--transport streamable-http` (+ `--host`/`--port`/`--allowed-host`, default stdio on 127.0.0.1, port 8008) so collaborators can share one instance with no checkout, plus `user="<login>"` on campaign_status/list_campaigns; `mine=true` is refused over HTTP because there the process account is the host, not the caller. systemd unit in `mcp/deploy/`. Live-verified on 127.0.0.1:8899; 1411 unit tests pass. `prodtools-write` stays stdio. See [[prodtools-mcp-server]].
- 2026-09-18 prodtools MCP servers migrated to mcp SDK 2.x (`mcp>=2,<3`): FastMCP -> MCPServer, host/port/transport_security moved to run(), sync tools now run in worker threads; found because an unpinned spec made fresh installs resolve to 2.2.0 and fail, and because all eight mu2eaigpvm01 servers run 2.x. Default HTTP port moved 8003 -> 8008 (8003 is arxiv-mcp). Dev venv rebuilt on ops python 3.12; 1412 tests, both --check, stdio smoke and live HTTP verified. See [[prodtools-mcp-server]].
- 2026-09-20 `stageEff` + MCP `stage_efficiency`: per-stage generated/kept counts and the chain product from art files (declared or not, SubRuns tree, no event loop) or SAM sums; prescales listed, undone only when named. Chosen over forwarding counts through the resampler after a probe MixFilter showed per-event SubRun ops, stale EventIDs with several input files, and empty subruns invisible to the mixer (0.315 vs true 0.252, NO_SUBRUN on wrap). Matches SAM exactly on MuminusStopsCat Run1Ban. See [[stage-efficiency-tool]].
