...
### Decisions
- [[2026-08-12-run1b-consolidation]] — Run1B folded into Offline `main` (PRs #1923, #1927, Production #565); `main` keeps the Run1A default and Run1B is selected by naming `geom_run1_b_v40.txt`. Production-scoped: v40 + degrader fix only; v01–v06 and their un-ported C++ stay on the `Run1B` branch, which is kept but frozen (deletion declined 2026-08-12). Run1Baa/Bab/Baf, Run1Bag/Bah and Run1Bai are **frozen** — v01 resolves on main but `inDS2Vacuum` is silently ignored, so re-running exits 0 with the wrong world. Reproduce via each entry's pinned CVMFS Musing _(2026-08-12)_
