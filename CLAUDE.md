...
`push_cnf` refuses them. A fourth, `run_local`, runs an entry (Musing or
code tarball) on this node instead: `runlocal` started detached under
the same receipt; `kill <pid>` stops it, jobs included.
