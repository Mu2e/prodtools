...
while [[ "$1" == -* ]]; do
  case "$1" in
    ...
    -t)
      TEST_MODE=true
      shift
      # Optional path argument for -t; a '-' flag or nothing left defaults to mktemp.
...
# Verify the release tag exists. -L is required: github.com archive URLs
# 302-redirect to S3, and without it a bare HEAD reports the redirect as
# a successful 3xx even for a nonexistent tag; -L follows through and
# fails cleanly on 404.
