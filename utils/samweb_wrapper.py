...
def _client() -> SAMWebClient:
    """The shared SAMWebClient. Experiment must resolve even on grid
    workers where SAM_EXPERIMENT may be unset (jobfcl's inner loop), so
    fall back to 'mu2e' explicitly rather than samweb_client's env-only
    default. `lru_cache(maxsize=1)` makes the lookup thread-safe
    (CPython's GIL + cache-result memoization).

    Until 2026-08-28 this module carried a class of ~24 methods PLUS 24
    one-line module-level delegates — two copies of a 48-symbol
    interface. The module functions ARE the interface now; tests inject
    a fake by patching _client.
    """
...
def create_definition(definition_name: str, query: str) -> None:
    ...
    _client().createDefinition(definition_name, query)
...
def delete_definition(definition_name: str) -> None:
    ...
    _client().deleteDefinition(definition_name)
...
def describe_definition(definition_name: str) -> str:
    ...
    return _client().descDefinition(definition_name)

def list_definition_files(definition_name: str, availability: str = "anylocation") -> List[str]:
    ...
    return _client().listFiles(_q_definition_files(definition_name, availability))
...
def file_sizes_in_dataset(dataset: str) -> Dict[str, int]:
    ...
    return {fi.file_name: fi.file_size
            for fi in _client().listFiles(dimensions=q, fileinfo=True)}
...
def get_metadata(filename: str) -> Dict:
    ...
    return _client().getMetadata(filename)
...
def definition_creation_date(defname: str) -> Optional[datetime]:
    ...
    info = None
    try:
        info = _client().descDefinitionDict(defname)
    except SAMError:
        pass
    ...
    try:
        text = describe_definition(defname)
    except SAMError:
        return None
...
def file_lineage(filename: str, lineage_type: str = 'parents') -> List[str]:
    ...
    result = _client().getFileLineage(lineage_type, filename)
...
def parents_of_file(filename: str) -> List[str]:
    ...
    with no parents'."""
    parents = _client().listFiles(_q_parents_of_file(filename))
...
def files_like(pattern: str, sequencer: Optional[str] = None) -> List[str]:
    ...
    return _client().listFiles(_q_dataset_like(pattern, sequencer))
...
def locate_file_strict(filename: str) -> List[Dict]:
    ...
    return _client().locateFile(filename)
...
def locate_files_strict(filenames: List[str]) -> Dict[str, List[Dict]]:
    ...
    for i in range(0, len(filenames), MAX_LOCATE_BATCH):
        out.update(_client().locateFiles(
