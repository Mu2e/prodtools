...
catalog (github.com/oksuzian/sim-epochs) derives a dataset's generation
from the cnf without a side table. Legacy datasets are backfilled once from the cnf
catalog itself: every cnf in SAM declares its outputs in
`tbs.outfiles`, and a generic `evnt` cnf claims the datasets carrying
its tier and dsconf (`epochs index-cnfs`,
`data/epochs/cnf_index.json` in the sim-epochs repository). Logs are not read.
...
Implemented 2026-09-11 in `runmu2e`: `JobRun.cnf` carries the tarball's
SAM name from every runner (art and g4bl); `push_data` appends it to
`parents_list.txt` after the inputs, and declares it alone when the
inputs are not SAM-registered (`inloc: dir:`, resampler jobs with no
inputs), where the third column used to read `none`; `push_logs` names
the same list for the log. A job with neither inputs nor cnf still
writes `none`.
